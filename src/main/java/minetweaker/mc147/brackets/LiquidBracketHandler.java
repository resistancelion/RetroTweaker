package minetweaker.mc147.brackets;
...
public class LiquidBracketHandler implements IBracketHandler {
	public static ILiquidStack getLiquid(String name) {
		LiquidStack liquid = LiquidDictionary.getLiquid(name, 1);
		if (liquid != null) {
			return new MCLiquidStack(liquid);
		} else {
			return null;
		}
	}
	...
	private IZenSymbol find(IEnvironmentGlobal environment, List<Token> tokens, int startIndex, int endIndex) {
		...
		LiquidStack liquid = LiquidDictionary.getLiquid(valueBuilder.toString(), 1);
		if (liquid != null) {
