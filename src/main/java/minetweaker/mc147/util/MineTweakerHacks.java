...
public class MineTweakerHacks {
	...
	public static Map getTranslations() {
		return getPrivateObject(
					getPrivateStaticObject(StatCollector.class, "localizedName", "a"),
					"languageList",
					"c");
	}
	...
	public static Map<List, LiquidContainerData> getLiquidContainerMap() {
		try {
			Field field = getField(LiquidContainerRegistry.class, new String[] {"mapFilledItemFromLiquid"});
			return (Map<List, LiquidContainerData>) field.get(null);
		} catch (IllegalAccessException ex) {
			MineTweakerAPI.logError("could not get liquid container registry");
			return null;
		}
	}
	
	public static Map<List, LiquidContainerData> getFilledContainerMap() {
		try {
			Field field = getField(LiquidContainerRegistry.class, new String[] {"mapFilledItemFromLiquid"});
			return (Map<List, LiquidContainerData>) field.get(null);
		} catch (IllegalAccessException ex) {
	...
	public static Set<List> getEmptyContainers() {
		try {
			Field field = getField(LiquidContainerRegistry.class, new String[] {"emptyContainers"});
